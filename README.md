...
2. [Key-Value Store](## 'Private POST | Public GET') mode (one-to-many and one-to-one):
   ...
   - **one-to-one:** If path is suffixed with any unique string: `<channel>`. POSTs to `https://securelay.tld/<private_path>/<channel>` is retained until next GET at `https://securelay.tld/<public_path>/<channel>` or till expiry, i.e. only a single member of the public can consume the data. Public POSTs to a channel (i.e. at `https://securelay.tld/<public_path>/<channel>`), stores the message in the private queue, with `channel=<channel>` as metadata.
